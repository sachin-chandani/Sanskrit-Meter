//June 29th, 2015, Modifying for the radio button as input in a different file
import java.util.regex.*;
import java.io.*;
public class test_mit_v4{
    ...
    public static int num_metre=1000;
    public static String metre_name[]=new String[num_metre];
    public static String metre_pat[]=new String[num_metre];
    public static int numSylVerse=0;
    public static int metre_entered=0;
    public static int numPatterns=0;
    public static String pats[]=new String[num_metre];
    public static String arDa_metre[]=new String[num_metre];
    public static String arDa_pat[]=new String[num_metre];
    public static int num_arDa=0;
    public static String vizama_metre[]=new String[num_metre];
    public static String vizama_pat[]=new String[num_metre];
    public static int num_vizama=0;
    public static String upajAti_metre[]=new String[num_metre];
    public static String upajAti_pat[]=new String[num_metre];
    public static int num_upajAti=0;
    public static int num_mAtrA_metre=0;
    public static String mAtrA_metre[]=new String[num_metre];
    public static String mAtrA_pat[]=new String[num_metre];
    public static int num_mAtrA_regex=0;
    public static String mAtrA_regex[]=new String[num_metre];
    public static String mAtrA_pat_all[]=new String[num_metre];
    public static String orig_input="";
    public static int radio_input=3;
    //[yrtBjsnm][rs][lg][lg] [yrtBjsnm][rs]l[lg] for anuztuB
    public static void main(String[] args) throws Exception{
	FileWriter fr=new FileWriter("anuztuB_output.txt");
	BufferedReader brm = new BufferedReader(new FileReader("samavftta.txt"));
	String s="";
	...
	brm.close();
	brm = new BufferedReader(new FileReader("arDasama.txt"));
	while((s=brm.readLine())!=null){
	    String tmp[]=s.trim().split(" ");
		...
		if(tmp[i].equals("vA")){
		    pats[numPatterns]=tmp[i-1];numPatterns++;
		}
		else{
	...
	brm.close();
	brm = new BufferedReader(new FileReader("vizama.txt"));
	while((s=brm.readLine())!=null){
	    String tmp[]=s.trim().split(" ");
	...
	brm.close();
	brm = new BufferedReader(new FileReader("upajAti.txt"));
	while((s=brm.readLine())!=null){
	    String tmp[]=s.trim().split(" ");
	...
	brm.close();
	brm = new BufferedReader(new FileReader("AryA_metre.txt"));
	while((s=brm.readLine())!=null){
	    String tmp[]=s.trim().split(" ");
	...
	brm.close();
	BufferedReader br = new BufferedReader(new FileReader ("anuzwuB_input.txt"));
	BufferedReader br1 = new BufferedReader(new FileReader ("radio_input.txt"));
	String si="";int vC=0;
	while((si=br.readLine())!=null){
	    radio_input=Integer.parseInt(br1.readLine().trim());//output should depend on tis input
	    /*
	      1  a single pAda, 
	      2. or two pAdas 
	      3. or full verse 
	      4. or more than one verse 
	      5. or verse with extra text.*/
	    numSylVerse=0;
	    vC++;
	    FileWriter fr1=new FileWriter("outputs/"+vC+".txt");
	    String longString = si.replace(" ","");
	    fr1.write("\nVerse number "+vC+"\t"+longString+"\n");
	    ...
	    if((indexDot>0)&&(indexDouble>indexDot)&&(radio_input==3)){//full verse case
		...
		try{
		    metre_here=main_metre(firstLine,secondLine,fr1);
		}
		catch (IOException e) {}
		if(!(metre_here.equals("none_found"))){isFound=true;}
		fr.write(metre_here);
	    }
	    else{//Assume that no dots are provided
		longString=longString.replace(".","");
		if((radio_input==1)||(radio_input==2)){//only one pAda is provided
		    ...
		    int sylInds[] = sylInd("[yvrlYmNRnJBGQDjbgqdKPCWTcwtkpSzsh]*[aAiIuUfFxXeEoO][HM]*", longString,numSyls);
		    loopO:for(int ii=0;ii<numSyls-1;ii++){
			numSylVerse=0;
			firstLine=longString.substring(0,sylInds[ii+1]);secondLine=longString.substring(sylInds[ii+1]);
			try{
			    metre_here=main_metre(firstLine,secondLine,fr1);
			}
			catch (IOException e) {}
			...
			if(numSyls>=16){
				...
				for(int kk=jj+1;kk<numSyls;kk++){//end
					...
					if((ii-jj>=15)&&(kk-ii>=15)){
					    numSylVerse=0;
					    firstLine=longString.substring(sylInds[jj],sylInds[ii+1]);secondLine=longString.substring(sylInds[ii+1],sylInds[kk]);
					    try{
						metre_here=main_metre(firstLine,secondLine,fr1);
					    }
					    catch (IOException e) {}
						...
						if(radio_input==4){
						    toCont=true;
						    longString=longString.substring(sylInds[kk]);
						}
						else{
						    toCont=false;
						}
	...
	public static String main_metre(String firstLine, String secondLine, FileWriter fr1) throws IOException{
	 ...
	 if((radio_input==1)||(radio_input==2)){//only first line has to be checked
	     int chars=0;
		 ...
		 chars=len_line1; lenSyl[numSyllable1-1] += len_line1-chars;
	     }
		 ...
		 if(metre_pAda[0].length()>0){
			 fr1.write(output_print+"The metrical patterns are...\n\n");
			 for(int i=0;i<1;i++){
			     fr1.write("......pAda "+(i+1)+":::::::::\n");
			     fr1.write("Parsed String     :\t"+metre_pAda[2]+"\n");
			 }
			 fr1.write("The verse is in "+metre_pAda[0]+" meter which is a samavftta.\n");
			 return metre_pAda[0];
		 }
		 else{//check for anu..
		     String pp1=make_optional_regex("ytBrjm");String pp2=make_optional_regex("ytBrnm");
		     Pattern pat1=Pattern.compile("^[lg]"+pp1+pp2+"[lg]$");
		     Matcher ma=pat1.matcher(metre_pAda[3]);
		     boolean isPres=true;
		     if(!ma.find()){fr1.write(metre_pAda[3]+"..is not in anuztuB\n");isPres=false;}
		     if(isPres){
			 fr1.write(output_print+"The metrical patterns are...\n\n");
			 for(int i=0;i<1;i++){
			     fr1.write("......pAda "+(i+1)+":::::::::\n");
			 ...
			 fr1.write("The verse may be in anuzwuB meter which is a samavftta.\n");
			 return "anuzwuB";
			 //metre_found=true;
		     }
		 }
		 ...
		 if(numSyllable1%2==0){//even number of syllables
			 ...
			 //System.out.println("pAda " + pAda + " " + metre_pAda[pAda][2]+"\nmetre_name..."+metre_pAda[pAda][0]+"\n");
		     }
		 }
		 ...
		 if(samavftta){
			 ...
			 for(int i=0;i<2;i++){
			     fr1.write("......pAda "+(i+1)+":::::::::\n");
			     fr1.write("Parsed String     :\t"+metre_pAda[i][2]+"\n");
			 }
			 fr1.write("The verse is in "+first_metre+" meter which is a samavftta.\n");
			 return first_metre;
			 //metre_found=true;
		     }
		 }
		 //check for anuztuB
		 //Pattern pat1=Pattern.compile("[yrtBjsnm][rs][lg][lg]");Pattern pat2=Pattern.compile("[yrtBjsnm][rs]l[lg]");
		 ...
		 Pattern pat1=Pattern.compile("^[lg]"+pp1+pp2+"[lg]$");Pattern pat2=Pattern.compile("^[lg]"+pp1+"lgl[lg]$");
		 Matcher ma=pat1.matcher(metre_pAda[0][3]);
		 boolean isPres=true;
		 if(!ma.find()){isPres=false;}//System.out.println("218");}
		 ma=pat2.matcher(metre_pAda[1][3]);
		 if(!ma.find()){isPres=false;}//System.out.println(metre_pAda[1][3]+"222");}
		 ...
		 if(isPres){
		     ...
		     fr1.write("The verse is in anuzwuB meter which is a samavftta.\n");
		     return "anuzwuB";
		     //metre_found=true;
		 }
		 loop_ar:for(int i=1;i<numSyllable1;i++){
		     int syl_pAda1=i;int syl_pAda2=numSyllable1-i;
		     int start=0;int numSyllable=0;
		     for(int pAda=0;pAda<2;pAda++){//4 pAdas
			 if(pAda%2==0){numSyllable=syl_pAda1;}
			 else{numSyllable=syl_pAda2;}
			 metre_pAda[pAda]=find_metre_pAda(lenSyl,syllables,numSyllable,start);
			 start+=numSyllable;
		     }
			 ...
			 String metre_here[] = find_metre_arDa(first_metre+" "+second_metre);
			 if(metre_here[0].length()>0){
			     fr1.write(output_print+"The metrical patterns are...\n\n");
			     for(int ii=0;ii<2;ii++){
				 fr1.write("......pAda "+(ii+1)+":::::::::\n");
				 fr1.write("Parsed String     :\t"+metre_pAda[ii][2]+"\n");
			     }
			     //System.out.println("The verse is in arDasamavftta metre pattern " + metre_here[0] +" with patterns "+metre_here[1]);
			     fr1.write("The verse is in "+metre_here[1]+ " meter which is a arDasamavftta.\n");
			     return metre_here[1];
			 }
		     }
		 }
	     }
	     fr1.write(output_print+"\n");
	     return  "none_found";
	 }
	 ...
	 if(chars<len_line1+len_line2){
	     ...
	     chars=len_line1+len_line2; lenSyl[numSylVerse-1] += len_line1+len_line2-chars;
	 }
	 ...
	 String metre_pAda[][]=new String[4][4];//4 different metres for each pAda, 3 strings, metre_name, gaRa_pattern, the output and the weights
	 ...
	 if(numSyllable1 == numSyllable2){
		 ...
		 if(!ma.find()){isPres=false;}//System.out.println("220");}
		 ma=pat2.matcher(metre_pAda[1][3]);
		 if(!ma.find()){isPres=false;}//System.out.println(metre_pAda[1][3]+"222");}
		 ma=pat2.matcher(metre_pAda[3][3]);
		 if(!ma.find()){isPres=false;}//System.out.println(metre_pAda[3][3]+"224");}
		 if(isPres){
		     metre_found=true;
		      fr1.write(output_print+"The metrical patterns are...\n\n");
		      for(int i=0;i<4;i++){
		 ...
		 //general loop to change the number of syllables and search for the pattern in files in arDasamavftta
	     loop_ar:for(int i=1;i<numSyllable1;i++){
		 int syl_pAda1=i;int syl_pAda2=numSyllable1-i;
		 int start=0;int numSyllable=0;
		 for(int pAda=0;pAda<4;pAda++){//4 pAdas
	     ...
	     //general loop to change the number of syllables and search for the pattern in files in vizamavftta
	 int syl_pAda[]=new int[4];
	loop_viz:for(int i=1;i<numSyllable1;i++){
	     syl_pAda[0]=i;syl_pAda[1]=numSyllable1-i;
	     for(int j=1;j<numSyllable2;j++){
		 syl_pAda[2]=j;syl_pAda[3]=numSyllable2-j;
		 int start=0;int numSyllable=0;
		 for(int pAda=0;pAda<4;pAda++){
		 ...
		 if(metre_here[0].length()>0){
		     ...
		     fr1.write("The verse is in " +metre_here[1]+ " meter which is a vizamavftta.\n");
		     return metre_here[1];
		     //metre_found=true;
		     //break loop_viz;
		 }
	     ...
	     //general loop to change the number of syllables and search for the pattern in files in upajAti
	 //int syl_pAda[]=new int[4];
	loop_upaj:for(int i=1;i<numSyllable1;i++){
		 ...
		 if(metre_here[0].length()>0){
		     ...
		     fr1.write("The verse is in " +metre_here[1]+ " meter which is a upajAti.\n");
		     return metre_here[1];
		     //metre_found=true;
		     //break loop_upaj;
		 }
	 ...
	 pat_pAda[1]=find_mAtrA_ArDa(lenSyl,syllables,numSyllable2,numSyllable1);
	 // System.out.println(pat_pAda[0]+" - mAtrA - "+pat_pAda[1]);
	 if((pat_pAda[0].length()>0)&&(pat_pAda[1].length()>0)){
	     // System.out.println(pat_pAda[0]+" - mAtrA - "+pat_pAda[1]);
	     String metre_here[]=find_metre_mAtrA(pat_pAda[0]+" "+pat_pAda[1]);
	     if(metre_here[0].length()>0){
		 metre_found=true;
		 fr1.write(output_print+"The verse is in " + metre_here[1]+ " meter which is a mAtrAvftta\n"); 
		 fr1.write("Parsed string     :\t");
		 for(int i=0;i<numSylVerse;i++){
		     fr1.write(syllables[i]+"  ");
		 }
		 fr1.write("\n");
		 return metre_here[1];
	     }
    ...
    public static String find_optional(String gaRa_pattern){
	return gaRa_pattern.substring(0,gaRa_pattern.length()-1)+"l";
    }
    public static boolean equates(String s1,String s2){
    ...
    public static String make_optional_regex(String pattern){
	String out="";
	for(int i=0;i<pattern.length();i++){
	    char search=pattern.charAt(i);
	    switch(search){
	    case 'y': out+="lgg|";break;
	    case 'r': out+="glg|";break;
	    case 't': out+="ggl|";break;
	    case 'B': out+="gll|";break;
	    case 'j': out+="lgl|";break;
	    case 's': out+="llg|";break;
	    case 'n': out+="lll|";break;
	    case 'm': out+="ggg|";break;
	    default:break;
	    }
	}
    ...
    public static String make_regex_mAtrA(String pattern,boolean optional_vA){
	String out="";
	for(int i=0;i<pattern.length();i++){
	    char search=pattern.charAt(i);
	    switch(search){
	    ...
	    case 'j':out+="(lgl)";break;
	    case 's':out+="(llg)";break;
	    case 'L':out+="(llll)";break;
	    case 'J':out+="(gg|gll|llg|llll)";break;
	    case '4':out+="(gg|gll|llg|lgl|llll)";break;
	    default:out+="("+search+")";break;
	    }
	}
	if(optional_vA){
    ...
    public static String[] find_metre_pAda(int lenSyl[],String syllables[],int numSyllable,int start){
	String metre_pAda="";String gaRa_pattern="";String output_print="";String vA_applied="f";String out_weights="";
	...
	for(int i=0;i<numSyllable;i++){
		...
		if(i==numSyllable-1){
		    ...
		    if(vowel_ind_next>=2){weight[i]=false;}//conjunct consonant
		    else{weight[i]=true;}
		}
	    }
	}
	...
	if(weight[numSyllable-1]){
	    ...
	    //gaRa_pattern.substring(0,gaRa_pattern.length()-1)+gaRa[numGroups-1];//we will check vA condition on this pattern
		...
		output_print+=("\n");
                output_print+=("Syllable weights  :\t");
		for(int i=0;i<numSyllable;i++){
		    output_print+=(out_weight(weight[i])+"  ");
		}
		output_print+="\n";
		output_print+=("Gana Pattern      :\t");
		for(int i=0;i<numGroups-1;i++){
		    output_print+=(gaRa[i]+" ");
		    gaRa_pattern+=gaRa[i];
		}
		...
		output_print+=("No. of Syllables  :\t"+numSyllable);
	...
	output_print+="\n";
	String output[]={metre_pAda,gaRa_pattern,output_print,out_weights,vA_applied};
	return output;
    }
     public static String find_mAtrA_ArDa(int lenSyl[],String syllables[],int numSyllable,int start){
	String metre_pAda="";String gaRa_pattern="";String output_print="";String vA_applied="f";
	boolean weight[]=new boolean[numSyllable];
	for(int i=0;i<numSyllable;i++){
	    String syl = syllables[start+i];
	    output_print+=(syl+" ");
    ...
    public static boolean find_vA(String gaRa_pattern){
    ...
    public static String find_metre(String[] gaRa){
	String pat="";
    ...
    public static int find_vowel_ind(String syl){
	...
	System.out.println("Error...no vowel in this syllable "+syl);
	return -1;
    }
    public static String[] find_metre_arDa(String pattern){
    ...
    public static String[] find_metre_mAtrA(String pattern){
    ...
    public static String[] find_metre_vizama(String pattern){
	//	System.out.println("pattern is "+pattern);
	String tmp[]=pattern.split(" ");
	...
	String pat="";String alt_pat="";
	for(int i=0;i<2;i++){
	    if(tmp1[0][i]!=null){
		pat=tmp1[0][i];
		alt_pat=pat.substring(0,pat.length()-1)+"l";
		for(int j=0;j<2;j++){
		    if(tmp1[1][j]!=null){
			pat=pat+" "+tmp1[1][j];
			alt_pat=alt_pat+" "+tmp1[1][j];
			for(int k=0;k<2;k++){
			    if(tmp1[2][k]!=null){
				pat=pat+" "+tmp1[2][k];
				alt_pat=alt_pat+" "+tmp1[2][k];
				for(int l=0;l<2;l++){
				    if(tmp1[3][l]!=null){
					pat=pat+" "+tmp1[3][l];
					alt_pat=alt_pat+" "+tmp1[3][l];
					for(int it=0;it<num_vizama;it++){
					    if(vizama_pat[it].equals(pat)){
						String output[]={pat,vizama_metre[it]};
						return output;
					    }
					    else if(vizama_pat[it].equals(alt_pat)){
						String output[]={alt_pat,vizama_metre[it]};
						return output;
					    }
					}
				    }
    ...
    public static String[] find_metre_upajAti(String pattern){
	String tmp[]=pattern.split(" ");
	...
	public static String out_weight(boolean weight){
        ...
        public static String gaRa_name(boolean[] weight){
            ...
            if(output.length()==0){
                System.out.println(" :For this pattern no metre is found"); //("For the pattern "+pat+" nothing found");
            }
            return output;
        }
	public static int regexChecker(String theRegex, String str2Check){
		...
		//Please note that we need numSyl for one line and numSyllable for the complete verse
    ...
    public static int[] sylInd (String theRegex, String str2Check, int numSyls){
